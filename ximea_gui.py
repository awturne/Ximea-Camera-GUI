...
class XimeaApp:
    ...
    def connect_and_start(self) -> None:
        ...
        try:
            ...
            self._disable_auto_preview_adjustments()
    ...
    def _set_image_format(self) -> str:
        ...
        for fmt_name in preferred:
            format_values = [fmt_name]
            if hasattr(xiapi, fmt_name):
                format_values.append(getattr(xiapi, fmt_name))
        ...
        param_candidates = ["imgdataformat", "image_data_format", "XI_PRM_IMAGE_DATA_FORMAT"]
        for fmt_name in preferred:
            format_values = [fmt_name]
            if hasattr(xiapi, fmt_name):
                format_values.append(getattr(xiapi, fmt_name))
        ...
        raise RuntimeError(
            "Could not set image format to XI_MONO16 or XI_RAW16. "
            f"Last XiAPI error: {last_error}"
        )
    ...
    def _disable_auto_preview_adjustments(self) -> None:
        if self.camera is None:
            return
        # Keep preview brightness fixed by disabling automatic camera-side
        # exposure/gain and LUT features when the XiAPI binding exposes them.
        try_methods = [
            ("set_aeag", 0),
            ("set_lut_en", 0),
            ("set_LUTEnable", 0),
        ]
        for method_name, value in try_methods:
        ...
        for param_name, value in try_params:
            try:
                self.camera.set_param(param_name, value)
            except Exception:
                pass
    ...
    def apply_camera_settings(self, show_message: bool = True) -> None:
        ...
        try:
            ...
            timing_mode_value = getattr(xiapi, "XI_ACQ_TIMING_MODE_FRAME_RATE", None)
            if timing_mode_value is not None:
                try:
                    self.camera.set_acq_timing_mode(timing_mode_value)
                except Exception:
                    pass
