...
- Live preview uses fixed scaling (no auto-LUT/auto-brightness): Mono12 DN values are mapped with a constant 0..4095 -> 0..255 scale.
- On connect/settings apply, the app also attempts to disable camera-side auto exposure/gain (`AEAG`) and LUT (`LUT_EN`) when supported by the XiAPI binding, to keep preview brightness fixed.
...
- On connect, the app now requires a deterministic 16-bit format and tries `XI_MONO16` then `XI_RAW16` (via direct setter and param fallback). If both are unsupported, connect fails with an explicit error instead of running with camera-default format.
- Applying camera settings uses direct XiAPI setters (`set_exposure`, `set_framerate`, `set_gain`) so rejected settings fail explicitly.
